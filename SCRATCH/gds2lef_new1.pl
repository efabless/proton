...
if($noOfArg < 6 || $_[0] eq '-h' || $_[0] eq '-help' || $_[0] eq '-HELP') {
   ...
   sub replace_sref_data{
     ...
     if(exists $SREF_DATA{$sname}){
        ...
        foreach my $line(@sref_data){
          ...
          &replace_sref_data($sname, $sname1, $angle1, $shiftX+$llx, $shiftY+$lly);
        }
     }
   }#sub replace_sref_data
   ...
   foreach my $poly (@data_arr){
     ...
     if(exists $boundary_layer_hash{$polygon[0]}){
        my $len = @{$boundary_layer_hash{$polygon[0]}};
        ...
        $poly_minX_hash{$polygon[0]}{$len} = $x_coords[0];
        $poly_minY_hash{$polygon[0]}{$len} = $y_coords[0];
        $poly_maxX_hash{$polygon[0]}{$len} = $x_coords[-1];
        $poly_maxY_hash{$polygon[0]}{$len} = $y_coords[-1];
        #-----------------------------------------------------#
     }
   }
   ...
   #my @p = @{$boundary_layer_hash{16}}; ##Global
   #foreach (@p){
   #  print "@$_\n";
   #}
   #my @p1 = @{$boundary_layer_hash{17}}; ##Global
   #foreach (@p1){
   #  print "@$_\n";
   #}
   #my @p2 = @{$boundary_layer_hash{18}}; ##Global
   #foreach (@p2){
   #  print "@$_\n";
   #}
   #exit;
   ...
   foreach my $l(keys %overlap_layer_hash){
      ...
      foreach my $maxY_poly_num (@maxY_keys){
          if($poly_arr[$maxY_poly_num] eq "" || !exists $MAXY{$maxY_poly_num}){next;}
          ...
          foreach my $minY_poly_num (@minY_keys){
              if($poly_arr[$minY_poly_num] eq "" || !exists $MINY{$minY_poly_num} || $maxY_poly_num == $minY_poly_num){next;}

              my $minx1 = $MINX{$minY_poly_num};
              my $miny1 = $MINY{$minY_poly_num};
              my $maxx1 = $MAXX{$minY_poly_num};
              my $maxy1 = $MAXY{$minY_poly_num};
              if($miny > $maxy1 || $maxx < $minx1 || $minx > $maxx1){next;}
              if($maxy < $miny1){last;}
          ...
          if($#overlapped_poly >= 0){
             ...
             push(@overlapped_poly,&get_overlap_poly(\@overlapped_poly_num, \@overlapped_poly));
   ...
   sub get_overlap_poly{
     ...
     foreach my $maxY_poly_num(@sub_poly_num_arr){
         ...
         if($#ovl_poly >= 0){
            ...
            delete $MAXX{$maxY_poly_num};
            delete $MAXY{$maxY_poly_num};
            push(@ovl_poly, &get_overlap_poly(\@ovl_poly_num, \@ovl_poly));
         }
     }
     return @ovl_poly;
   }#sub get_overlap_poly
   ...
   foreach my $layer (sort{$a<=>$b}keys %overlap_layer_hash){
     ...
     for(my $i=0; $i<=$#layer_group; $i++){
         ...
         push(@group_poly, &get_hier_overlap($layer, $layer_group[$i], $layer));
   ...
   sub get_hier_overlap {
     ...
     foreach my $poly (@$group){
        ...
        for(my $j=0; $j<=$#via_poly_arr; $j++){
            ...
            if($result_arr_len > 0){
               ...
               delete $boundary_layer_hash{$via_layer}[$j];
     ...
     for(my $i=0; $i<=$#via_overlapped; $i++){
         ...
         LOOPA:for(my $j=0; $j<=$#second_lgrp; $j++){
             if($second_lgrp[$j] eq ""){next;}
             foreach my $polygon (@{$second_lgrp[$j]}){
   ...
   sub get_rev_hier_overlap {
     ...
     foreach my $poly (@$group){
        ...
        for(my $j=0; $j<=$#rev_via_poly_arr; $j++){
            ...
            if($result_arr_len > 0){
               ...
               push(@group_poly, [@clip_poly]);
               delete $boundary_layer_hash{$rev_via_layer}[$j];
     ...
     for(my $i=0; $i<=$#rev_via_overlapped; $i++){
         ...
         LOOPB:for(my $j=0; $j<=$#first_lgrp; $j++){
             if($first_lgrp[$j] eq ""){next;}
             foreach my $polygon (@{$first_lgrp[$j]}){
               my @clip_poly = @$polygon;
               my @p1 = ();
               for(my $k=1; $k<=$#clip_poly; $k=$k+2){
                   push(@p1,[$clip_poly[$k],$clip_poly[$k+1]]);
               ...
               if($result_arr_len > 0){
                  ...
                  if($starting_layer == $first_layer){
                     delete $layer_group_hash{$starting_layer}[$j];
                  }
   ...
   print "Making final group complete :",timestr($td3),"\n";
   ...
   print "Making pin group complete :",timestr($td4),"\n";
